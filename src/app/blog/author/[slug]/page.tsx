...
import { getBlogsByAuthorName } from "@/lib/blogs";
...
export default async function AuthorBlogsPage({ params }: PageProps) {
  const { slug } = await params;
  const authorName = decodeURIComponent(slug);

  const fromDb = await getBlogsByAuthorName(authorName).catch(() => []);
  const fromMock = mockBlogPosts.filter((p) => p.author === authorName);
  const blogs = [
    ...fromDb.map((p) => ({
      id: p.id,
      title: p.title,
